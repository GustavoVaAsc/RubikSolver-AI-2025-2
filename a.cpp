...
// Function to draw loading text on screen
void drawLoadingScreen(SDL_Renderer* renderer, TTF_Font* font) {
    ...
    SDL_Surface* textSurface = TTF_RenderText_Solid(font, "Cargando...", whiteColor);
    SDL_Texture* textTexture = SDL_CreateTextureFromSurface(renderer, textSurface);
    SDL_FreeSurface(textSurface);

    SDL_Rect textRect = { 350, 300, 200, 50 };
    SDL_RenderCopy(renderer, textTexture, NULL, &textRect);
    SDL_DestroyTexture(textTexture);

    // Simulate a loading animation (you can add more things here)
...
int main() {
    ...
    // Load font for loading text
    TTF_Font* font = TTF_OpenFont("creatyo.ttf", 24);
    if (font == nullptr) {
    ...
    int startX = (900 / 2) - (40 * 3.5);
    int startY = (700 / 2) - (40 * 4);
    ...
    while (running) {
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (isButtonHovered(buttonX, buttonY, buttonSize, mouseX, mouseY)) {
                    ...
                    // After "loading", reset and show the cube again
                    isLoading = false;
                    showCube = true;
                }
            }
        ...
        if (isLoading) {
            // Draw loading screen
            drawLoadingScreen(renderer, font);
        } else if (showCube) {
            // Here you would draw the cube (if not loading)
            // For example:
            // drawFace(renderer, startX, startY, 40, cube[0]);  // Draw the first face
        }

        // Render the button
        SDL_Rect buttonRect = { buttonX, buttonY, buttonSize, buttonSize };

        if (isButtonHovered(buttonX, buttonY, buttonSize, event.button.x, event.button.y)) {
